...
Для взаимодействия с базой данных используется Spring Data JPA. Созданы интерфейсы-репозитории, которые наследуются от `JpaRepository`. Это позволяет Spring автоматически предоставлять реализацию стандартных CRUD-операций без написания SQL-кода вручную. Все репозитории находятся в пакете `ru.university.lab3.repository`.
...
\begin{itemize}
    \item \textbf{@Entity (JPA)}: Указывает, что класс является сущностью и его объекты должны сохраняться в базе данных. Каждый экземпляр этого класса соответствует строке в таблице.
    \item \textbf{@Table(name = "...") (JPA)}: Явно задаёт имя таблицы в БД, с которой связан данный класс.
    \item \textbf{@Id (JPA)}: Помечает поле как первичный ключ таблицы.
    \item \textbf{@GeneratedValue (JPA)}: Определяет стратегию генерации значения для первичного ключа. В данном проекте используется стратегия по умолчанию, которая для PostgreSQL означает использование последовательностей (sequences).
    \item \textbf{@Inheritance(strategy = InheritanceType.JOINED) (JPA)}: Настраивает стратегию наследования. Стратегия `JOINED` означает, что для каждого класса в иерархии (базового и дочерних) создаётся своя таблица. Таблицы дочерних классов связаны с таблицей базового класса по внешнему ключу, которым является первичный ключ.
    \item \textbf{@OneToMany и @ManyToOne (JPA)}: Определяют реляционную связь "один ко многим" между сущностями. В классе `Organization` аннотация `@OneToMany` указывает на список сотрудников. В классе `Employees` аннотация `@ManyToOne` указывает на организацию, к которой принадлежит сотрудник.
    \item \textbf{@JoinColumn(name = "...") (JPA)}: Указывает имя столбца в таблице, который является внешним ключом для связи.
    \item \textbf{@Repository (Spring)}: Аннотация, которая помечает интерфейс репозитория. Spring Data JPA сканирует такие интерфейсы и автоматически создает их реализации во время выполнения.
    \item \textbf{@Autowired (Spring)}: Используется в контроллере для автоматического внедрения зависимостей, в данном случае — экземпляров репозиториев, что позволяет вызывать методы для работы с БД.
\end{itemize}
